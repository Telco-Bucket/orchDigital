...
export default class DeleteConsultList extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:4001/consult_form/'+this.props.match.params.id)
            .then(response => {
                this.setState({
    ...
    onSubmit(e) {
        ...
        axios.delete('http://localhost:4001/consult_form/'+this.props.match.params.id)
            .then(res => console.log(res.data));

        this.props.history.push('/consultformtable');
           ...
           <h5>This will be deleted permenently from  your database</h5>
              <h3>Are You sure You want continue this process??? </h3>
                ...
                <form onSubmit={this.onSubmit} >
                       ...
                       <Link type='button' className="btn btn-success  w-25" to="/admin" style={{color:"white"}}>NO</Link>
              
             
                            <input type="submit" value="DELETE" className="btn btn-danger w-25" />
                        </div>
                    {/* </div> */}
                </form>
                </section>
