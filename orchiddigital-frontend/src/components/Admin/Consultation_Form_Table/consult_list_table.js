...
componentDidMount() {
    axios.get('http://localhost:4001/consult_form/')
        .then(response => {
...
contactFormList() {
    return this.state.infos.map(function(currentinfos, i) {
        return <ConsultForm info={currentinfos} key={i} />;
    });
}
